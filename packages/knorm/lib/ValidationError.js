...
class ValidationError extends KnormError {
  ...
  formatMessage({ field, validator = {} }) {
    ...
    if (validator.required) {
      ...
      message = `Expected value of min-length \`${
        validator.minLength
      }\` ${forFieldName}`;
    } else if (validator.maxLength !== undefined) {
      message = `Expected value of max-length \`${
        validator.maxLength
      }\` ${forFieldName}`;
    } else if (validator.oneOf) {
      ...
      message = `Expected value ${forFieldName} ${
        validator.notMatching ? 'not ' : ''
      }to match ${inspect(validator.regex)}`;
    } else if (validator.validate) {
