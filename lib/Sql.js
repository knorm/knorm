...
const { asArray, inspectInstance } = require('./util');
...
class Sql {
  ...
  format() {
    return { text: this.formatText(), values: this.getValues() };
  }
  ...
  formatField(field) {
    ...
    const qualifier =
      options && options.qualifier
        ? this.formatIdentifier(options.qualifier)
  ...
  formatSql(sql) {
    const text =
      sql instanceof this.constructor ? sql.formatValue() : sql.formatText();
    ...
    return this.formatExpression(expression, {
      formatObject: object => {
        ...
        for (const [field, value] of Object.entries(object)) {
          expressions.push(
            `${this.formatField(field)} = ${this.formatExpression(value)}`
          );
        }
  ...
  createParts(queryOptions) {
    if (queryOptions instanceof QueryOptions) {
      queryOptions = queryOptions.getOptions();
    }
