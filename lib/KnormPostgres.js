...
class KnormPostgres {
  ...
  updateTransation(knorm) {
    ...
    knorm.Transaction = class PostgresTransaction extends Transaction {
      ...
      async releaseClient() {
        return knormPostgres.releaseClient(this.client);
      }
        ...
        try {
          ...
          try {
            ...
            try {
              ...
              const error = new this.constructor.TransactionError(
  ...
  updateQuery(knorm) {
    ...
    class PostgresQuery extends Query {
      ...
      // TODO: throw if data has no primary key
      // TODO: use super.prepareUpdateBatch when batch has only one item? in this
      // case all the tests for updates will need to update more than one item
