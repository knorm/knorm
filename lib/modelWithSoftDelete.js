module.exports = (Base, config = {}) => {
  ...
  class ModelWithSoftDelete extends Base {
    async restore(options) {
      return this._initQuery(options).restore();
    }
    ...
    async hardDelete(options) {
      ...
      // TODO: maybe through if the object didn't have an id to avoid more than
      // one row.

      if (rows.length === 1) {
        return this.setData(rows[0]);
      } else {
        // TODO: attempt to support LIMIT 1 for hardDelete()
