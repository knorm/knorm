...
class Field {
  constructor(config = {}) {
    ...
    if (validate && typeof validate !== 'function') {
      throw new Error(
        `Custom validator for field ${field} should be a function`
      );
    }
    ...
    if (cast) {
      if (cast.forSave) {
        if (typeof cast.forSave !== 'function') {
          throw new Error(
            `Pre-save cast function for field ${field} should be a function`
          );
        }
      ...
      if (cast.forFetch) {
        if (typeof cast.forFetch !== 'function') {
          throw new Error(
            `Post-fetch cast function for field ${field} should be a function`
          );
        }
        this.castors = this.castors || {};
    ...
    if (references) {
      ...
      this._updateReferences(references);
    }
  ...
  validateIsAny(value, type) {
    this.validateTypeWith(value, type, () => true);
  }

  validateIsNumber(value, type) {
    this.validateTypeWith(value, type, value => typeof value === 'number');
  }
  ...
  validateIsText(value, type) {
    this.validateIsString(value, type);
  }
  ...
  validateIsDateTime(value, type) {
    this.validateIsDate(value, type);
  }
  ...
  validateMatchesRegex(value, regex) {
    if (!isSet(value)) {
      return true;
    }
  ...
  async validateSchemaMatches(value, schema, modelInstance) {
    // TODO: validateSchemaMatches => validateWithSchema
    // TODO: array types
    // TODO: object types
    // TODO: itemSchema
    if (!isSet(value)) {
  ...
  async validateWithValidators(value, validators, modelInstance) {
    ...
    if (regex) {
      this.validateMatchesRegex(value, regex);
    }

    if (schema) {
      await this.validateSchemaMatches(value, schema, modelInstance);
    }
  ...
  updateModel(model) {
    ...
    if (this.validators.schema) {
      ...
      if (schema) {
        Object.values(schema).forEach(field => field.updateModel(model));
      }
    }
