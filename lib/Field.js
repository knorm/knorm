...
class Field {
  ...
  constructor(config = {}) {
    ...
    this.displayName = `\`${model.name}.${path}\``;

    if (!type) {
      throw new Error(`Field ${this.displayName} has no type configured`);
    }

    if (!this.constructor.types.includes(type)) {
      throw new Error(
        `Field ${this.displayName} has an invalid type \`${type}\``
      );
    }

    if (validate && typeof validate !== 'function') {
      throw new Error(
        `\`validate\` option for field ${this.displayName} should be a function`
      );
    }
  ...
  _createVirtual({ get, set }) {
    if (!get && !set) {
      throw new Error(
        `Virtual field ${this.displayName} has no \`get\` or \`set\` function`
      );
    }
    ...
    if (get) {
      if (typeof get !== 'function') {
        throw new Error(
          `Getter for virtual field ${this.displayName} is not a function`
        );
      }
      this.virtual.get = get;
    }

    if (set) {
      if (typeof set !== 'function') {
        throw new Error(
          `Setter for virtual field ${this.displayName} is not a function`
        );
      }
      this.virtual.set = set;
    }
  }
  ...
  /**
   * Checks the the {@link Field}'s configuration has a default value set, or a
   * function that returns the default value.
   *
   * @returns {boolean} `true` if the field has a default, `false` otherwise.
   */
  hasDefault() {
