...
class KnormRelations {
  ...
  updateQuery(knorm) {
    ...
    class RelationsQuery extends Query {
      ...
      addJoin(type, joins, options) {
        ...
        joins.forEach(join => {
          ...
          const parsedReferences =
            joinReferences[this.model.name] || thisReferences[join.model.name];
          ...
          join.config.isForwardJoin = !!joinReferences[this.model.name];
          join.config.parsedReferences = parsedReferences;
          join.config.parsedReferencesByTo = mapReferencesByTo(
            parsedReferences
          );
      ...
      prepareOn() {
        let references = [];
        const isForwardJoin = this.config.isForwardJoin;
          ...
          this.options.on.forEach(field => {
            if (field instanceof Field) {
              if (field.model === this.parent.model) {
                if (isForwardJoin) {
                  // treated as a reversed join
                  references.push(
                    ...this.config.parsedReferencesByTo[field.name]
                  );
                } else {
                  // treated as a forward join
                  references.push(this.config.parsedReferences[field.name]);
                }
                // references already handled (as the opposite kind), exit
                return;
              }
            ...
            if (isForwardJoin) {
              references.push(this.config.parsedReferences[field]);
            } else {
              references.push(...this.config.parsedReferencesByTo[field]);
            }
          });
        } else {
          references = Object.values(this.config.parsedReferences);
        }
