...
class Knorm {
   ...
   * @param {boolean|object} config.debug whether or not to enable debug mode.
   * If set to `true`, then the default debug config is set, which is:
   * ```js
   * {
   *   stackTraces: true,
   *   errorSqlValues: true,
   ...
   * }
   * ```
   * See [the debugging guide](/guides/debugging.html) for more info.
   *
   */
  constructor(config = {}) {
    this.plugins = {};
    ...
    this.config.debug = this.createDebugConfig(this.config.debug);
  ...
  /**
   * Creates a debug config, given a user-supplied debug config.
   *
   * @param {boolean|object} debug the user-supplied debug config.
   *
   * @returns {object} the created debug config.
   *
   * @private
   */
  createDebugConfig(debug) {
    const defaultConfig = {
      stackTraces: true,
      errorSqlValues: true,
      logSql: false
    ...
    if (typeof debug === 'object') {
      return Object.assign(defaultConfig, debug);
    }
  }
  ...
  logSql(sql) {
    ...
    let message = new Date().toISOString() + ' knorm:sql: ';

    if (typeof sql === 'string') {
      message += sql;
    } else if (sql instanceof this.Query.prototype.sql.Statement) {
