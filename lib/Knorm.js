...
class Knorm {
  constructor({ knex, fieldToColumn } = {}) {
    ...
    if (!knex) {
      throw new this.KnormError('Knorm: no knex instance provided');
    }

    if (!knex.client || !(knex.client instanceof KnexClient)) {
      throw new this.KnormError('Knorm: invalid knex instance provided');
    }
