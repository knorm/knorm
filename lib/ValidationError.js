...
class ValidationError extends KnormError {
  ...
  formatMessage({ field, validator = {} }) {
    const forFieldName = `for field \`${field.model.name}.${field.path}\``;
    let message;
    ...
    if (validator.required) {
      ...
      message = `Expected value ${validator.forFieldName} to match ${inspect(
        validator.regex
      )}`;
    } else if (validator.schema) {
      // TODO: add more info to this error message
      message = `Expected value ${forFieldName} to match json schema`;
    } else if (validator.validate) {
