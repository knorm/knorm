...
class Transaction {
    ...
    this.models = Object.entries(this.models).reduce(
      (models, [name, model]) => {
        ...
        models[name].Query = class extends model.Query {
          ...
          close() {
            // let Transaction handle connection closing
          }
        };
   ...
   * closes the database connection, via {@link Transaction#close}. If the
  ...
  async commit() {
    ...
    // do not close within the try..catch so that connection closing errors
   ...
   * closes the database connection, via {@link Transaction#close}.
   *
   * @returns {Promise} A `Promise` that is resolved when the transaction is
