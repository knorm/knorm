...
class Transaction {
  constructor(callback) {
    ...
    Object.entries({
      ...
      transaction[className] = scopedClass;
    });

    TransactionModel.Field = TransactionField;
    TransactionModel.Query = TransactionQuery;

    Object.entries(transaction.models).forEach(([name, model]) => {
      ...
      [models[name], TransactionField, TransactionQuery].forEach(
        scopedClass => {
          scopedClass.prototype.models = scopedClass.models = models;
          scopedClass.prototype.transaction = scopedClass.transaction = transaction;
        }
