...
class Model {
  ...
  // TODO: these methods don't fetched cast data since they disable forging
  async fetch(options) {
  ...
  static addField(field) {
    ...
    this._config.columnsToFields[field.column] = name;
  }

  // depended on by knorm-relations
  static removeField(field) {
    const { name, column, methods, primary, unique } = field;
    ...
    delete this._config.columnsToFields[column];
