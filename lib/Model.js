...
const castOrParseValues = (model, fields, castFunction) => {
...
class Model {
   ...
   */
  setDefaults({ fields } = {}) {
    fields = fields || this.$config.defaulted;
   ...
   */
  castValues({ fields } = {}) {
    fields = fields || this.$config.cast;

    castOrParseValues(this, fields, 'castValue');
   ...
   */
  parseValues({ fields } = {}) {
    fields = fields || this.$config.parsed;

    castOrParseValues(this, fields, 'parseValue');
  ...
  static addField(fieldConfig) {
    ...
    if (field.default !== undefined) {
      config.defaulted.push(name);
    }

    if (castValue) {
      config.cast.push(name);
    }

    if (parseValue) {
      config.parsed.push(name);
    }
  ...
  static removeField(field) {
    ...
    if (field.default !== undefined) {
      removeArrayItem(config.defaulted, name);
    }

    if (castValue) {
      removeArrayItem(config.cast, name);
    }

    if (parseValue) {
      removeArrayItem(config.parsed, name);
    }
