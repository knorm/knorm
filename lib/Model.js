class Model {
  ...
  _initQueryWithWhere(options = {}, operation) {
    ...
    query.where({ [this.constructor.fieldNames.id]: id });

    return query;
  }

  async fetch(options) {
    const row = await this._initQueryWithWhere(options, 'fetch')
      .first(true)
      .forge(false)
      .fetch();
  ...
  async delete(options) {
    const rows = await this._initQueryWithWhere(options, 'delete')
      .forge(false)
      .delete();
  ...
  async update(options) {
    const rows = await this._initQueryWithWhere(options, 'update')
      .forge(false)
      .update(this);
