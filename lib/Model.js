  ...
  static _addFields(fields, { validate = true } = {}) {
    Object.keys(fields).forEach(name => {
      ...
      if (field.methods && (field.primary || field.unique)) {
        const formatedName = upperFirst(name);

        this[`fetchBy${formatedName}`] = async function(value, options) {
          return new this({ [name]: value }).fetch(options);
        };

        this[`updateBy${formatedName}`] = async function(value, data, options) {
          data = Object.assign({}, data, { [name]: value });
          return new this(data).update(options);
        };

        this[`deleteBy${formatedName}`] = async function(value, options) {
