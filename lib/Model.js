class Model {
  ...
  _initQueryWithWhere(options = {}) {
    const query = this._initQuery(options);
    const id = this[this.constructor.fieldNames.id];

    if (id !== undefined) {
      // TODO: the best thing is to throw if an id is not set. bad things
      // could happen with new Model().delete() or new Model().update().
      // maybe introduce a "strict mode" config option
      const where = { [this.constructor.fieldNames.id]: id };
      query.where(where);
    }
  ...
  async fetch(options) {
    const row = await this._initQueryWithWhere(options)
      .first(true)
      .forge(false)
      .fetch();
  ...
  async delete(options) {
    const rows = await this._initQueryWithWhere(options)
      .forge(false)
      .delete();
    ...
    // TODO: there really shouldn't be more than one row deleted, but there could
    // be if the object didn't have an id. best thing might be to throw if that's
    // the case
  ...
  async update(options) {
    ...
    // TODO: there really shouldn't be more than one row updated, but there could
    // be if the object didn't have an id. best thing might be to throw if that's
    // the case
    ...
    if (rows.length === 1) {
      ...
      // TODO: attempt to support LIMIT 1 for update()
      return rows.map(row => new this.constructor(row));
    }
  }
