class Model {
  ...
  async _initQueryWithWhere(options = {}) {
    const query = this._initQuery(options);

    if (options.where === undefined) {
      const idField = this.constructor.fieldNames.id;
  ...
  async fetch(options) {
    const query = await this._initQueryWithWhere(options);
  ...
  // TODO: add Model.count and support passing all options to Query.prototype.count
...
const createVirtuals = model => {
  ...
  if (model._virtualsModelName !== model.name) {
    model._virtuals = Object.values(
      ...
      virtuals[virtual.name] = virtual.clone().setModel(model);
      return virtuals;
    }, {});
