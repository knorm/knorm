...
class Model {
  constructor(data = {}) {
    ...
    Object.defineProperties(
      ...
      Object.assign(
        {
          // this is so named to avoid conflicts with user-provided field names
          _knorm: {
            value: {
              config: config,
              unique: unique,
  ...
  // depeded on by knorm-relations
  static addFieldToConfig(field, modelConfig) {
    const name = field.name;
    const model = this;

    if (field.primary) {
      modelConfig._primary = name;
    } else if (modelConfig._primary === name) {
      modelConfig._primary = undefined;
    }

    if (field.updated === false) {
      modelConfig.notUpdated.push(name);
    } else {
      const index = modelConfig.notUpdated.indexOf(name);
      if (index > -1) {
        modelConfig.notUpdated.splice(index, 1);
      }
    }

    if (field.unique) {
      modelConfig.unique.push(name);
    } else {
      const index = modelConfig.unique.indexOf(name);
      if (index > -1) {
        modelConfig.unique.splice(index, 1);
      }
    }
    ...
    modelConfig._fields[name] = field;
    modelConfig.fieldsToColumns[name] = field.column;
    modelConfig.columnsToFields[field.column] = name;
  }
  ...
  // TODO: add `where` static proxying to Query.Where
