...
class Model {
  ...
  constructor(data = {}) {
    ...
    const descriptors = {
      _config: {
        value: { config, unique: config.unique, fields: config.fields }
      }
    };

    // TODO: avoid looping through all Fields in the Model constructor?
  ...
  static createField(config) {
    // TODO: refactor Field.prototype.model to Field.prototype.Model
    config = Object.assign({}, config, { model: this });
    // TODO: pass Model as the first arg?
    return new this.Field(config);
  }
  ...
  static set config({ schema, table, fields, options }) {
    if (!this._config) {
