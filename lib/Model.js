...
const prepareOrParseValues = async (model, fields, method) =>
  Promise.all(
    fields.map(async fieldName => {
      ...
      if (value !== undefined && field[method]) {
        const newValue = await field[method](model, value);
...
class Model {
   ...
   * returned object. If not set, all values set on the instance is returned.
   *
   ...
   */
  async prepareValues({ fields } = {}) {
    fields = fields || this.$config.fields.prepared;

    await prepareOrParseValues(this, fields, 'prepareValue');
  ...
  async parseValues({ fields } = {}) {
    ...
    await prepareOrParseValues(this, fields, 'parseValue');
  ...
  static getDefaultConfig() {
    ...
    defaultConfig.options = {
      query: {
        ...
        debug: defaultConfig.Model.knorm.config.debug,
        from: defaultConfig.Model,
        fields: defaultConfig.fields.names
      }
    };
  ...
  static get query() {
    return new this.Query(this).setDefaultOptions(this.config.options.query);
  }
