...
class Model {
  ...
  static addField(field) {
    const { name, virtual, column, primary, updated, unique, methods } = field;

    if (typeof this.prototype[name] !== 'undefined') {
      return this.throwModelError(
        `field \`${name}\` conflicts with \`${this.name}.prototype.${name}\``
      );
    }
  ...
  static removeField(field) {
    const { name, methods, primary, unique, updated, virtual } = field;
    const config = this.config;

    if (!config._fields[name] || config._fields[name] !== field) {
      return this.throwModelError(`unkown field \`${name}\``);
    }
