...
class Query {
  ...
  constructor(model) {
    ...
    this.options = {
      forge: true,
      debug: Object.assign({}, this.knorm.config.debug)
    };
    this.config = {
      table,
      index: 0,
  ...
  // TEST: chaining
  // TEST: not tampering with knorm's debug config
  // TEST: merging debug into default debug config
  debug(debug) {
    Object.assign(this.options.debug, this.knorm.createDebugConfig(debug));
    return this;
  }
   ...
   *
   * @example Using raw SQL (with PostgreSQL):
   * ```js {10}
   * Model.insert(
   *   {
   *     firstName: 'Foo',
   ...
   * database.
   */
  async beforeQuery(client, sql) {
    this.knorm.logSql(sql);
  }
