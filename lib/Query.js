...
class Query {
  ...
  constructor(model) {
    ...
    this.queryOptions = new this.constructor.QueryOptions();
  }
  ...
  setOption(option, value) {
    if (typeof this.queryOptions[option] !== 'function') {
      throw new this.constructor.QueryError(
        `${this.model.name}: unknown option \`${option}\``
      );
    }

    value = this.queryOptions[option](value);
    ...
    if (value === null) {
      ...
      this.options[option].concat(value);
    } else {
  ...
  /**
   * Updates the {@link QueryOptions} class used by {@link Query}. This enables
   * adding custom query options or overriding the exising query options.
   *
   * @param {QueryOptions} QueryOptions The new class. This could be a class
   * that extends the current {@link Query.QueryOptions} class or an entirely
   * new class.
   *
   * @returns {Query} The {@link Query} class, to allow chaining.
   */
  static updateQueryOptions(QueryOptions) {
    /**
     * The {@link QueryOptions} class used by all {@link Query} instances.
     *
     * @type {QueryOptions}
     */
...
Query.updateQueryOptions(require('./QueryOptions'));
