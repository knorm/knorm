...
class Query {
  ...
  constructor(model) {
    ...
    // TODO: move this check to getTable
    if (!table) {
      throw new this.constructor.QueryError(
    ...
    // TODO: refactor this.model => this.Model
    this.model = model;
  ...
  getTable() {
    // TODO: strict mode: throw if no table is set
  ...
  getColumn(field) {
    ...
    return this.model.config.fieldsToColumns[field];
  }
  ...
  /**
   * Updates the {@link Options} class used by {@link Query}. This enables
   * adding custom query options or overriding the exising query options.
   *
   * @param {Options} Options The new class. ideally one that  extends the
   * current {@link Query.Options} class.
   *
   * @returns {Query} The {@link Query} class, to allow chaining.
   */
  ...
  /**
   * Updates the {@link Sql} class used by {@link Query}. This enables adding
   * custom methods overriding exising ones.
   *
   * @param {Sql} Sql The new class, ideally one that extends the current
   * {@link Query.Sql} class.
   *
   * @returns {Query} The {@link Query} class, to allow chaining.
   */
  static updateSql(Sql) {
    /**
     * The {@link Sql} class used by {@link Query} instances.
     *
     * @type {Sql}
     */
    this.Sql = Sql;
...
// TODO: remove? what about Model.Where and Field.Where?
Query.Where = require('./Query/Where');

// TODO: test this:
Query.updateSql(require('./Query/Sql'));
