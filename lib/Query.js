...
class Query {
  ...
  // TODO: remove support for fields altogether?
  distinct(...fields) {
    if (fields[0] === false) {
  ...
  async prepareSql(sql) {
    if (this.config.forInsert || this.config.forTruncate) {
      return sql;
    }
  ...
  async prepareTruncate(options) {
    this.setOptions(options);

    this.config.forTruncate = true;
  ...
  async truncate(options) {
    const stack = this.options.debug ? new Error().stack : undefined;
    const sql = await this.prepareTruncate(options);
    await this.execute(sql).catch(error => {
      throw this._attachErrorStack(
        new this.constructor.TruncateError({ error, query: this }),
        stack
      );
    });
  }
...
/**
 * The rejection error from {@link Query#truncate} on error.
 *
 * @extends {Query.QueryError}
 */
Query.TruncateError = class TruncateError extends Query.QueryError {};
