...
class Query extends WithKnex {
    ...
    async _parseData(data) {
        ...
        if (!parsedData) {
            ...
            parsedData = new this.model(instanceData);

            // TODO: test post-fetch casting with fields configured
            parsedData.cast({ fetch: true });

            if (this._forge === false) {
                parsedData = await parsedData.getData();
            }
        ...
        if (this._with.length) {
            this._with.forEach(async query => {
                if (query._hasData(data)) {
                    const nestedData = await query._parseData(data);
    ...
    async fetch() {
        ...
        const parsedRows = await this._parseRows(rows);
