...
class Query extends WithKnex {
  ...
  distinct(field) {
    this._distinct = this._getValidatedField(field);
  }
  ...
  _addCount() {
    if (this._distinct) {
      this.builder.countDistinct(this._getColumn(this._distinct));
    } else if (this._field) {
  ...
  _prepareBuilder(options = {}) {
    ...
    const { forCount, forInsert, forUpdate, forDelete } = options;
  ...
  async insert(data, options) {
    ...
    const prepareData = async data => {
      ...
      // TODO: casting should be done after validation
      // TODO: improve handling of json fields. the lack of proper support is
      // essentially forcing us to cast before validation
      // TODO: support json schema (for validation)
      // TODO: support jsonb fields
      // TODO: support email fields
