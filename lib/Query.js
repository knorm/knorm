...
class Query {
  ...
  formatSelect(options) {
    ...
    // TODO: add support for options scoped by method i.e. `fetch` option
  ...
  formatInsert(data, options) {
    // TODO: handle batched inserts i.e. with a `batchSize` option
    ...
    const batches = batchSize ? this.createBatches(batchSize) : [data];

    return batches.map(batch => {
      const batchOptions = Object.assign({});
      const insert = this.prepareInsert(batchOptions);
      const sql = this.Model.sql;
  ...
  async prepareData(data) {
    ...
    await Promise.all(
      data.map(async data => {
        // TODO: remove getInstance and always create a new instance
