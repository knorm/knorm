...
class Query {
  ...
  /**
   * Configures the `FROM` clause in a query. This is handy for fetching with
   * sub-selects or from multiple tables (if the database supports it).
   *
   * ::: warning NOTE
   * If the {@link Query#from} option is configured, the {@link Query#fields}
   * option should also be configured, with the same alias as the
   * {@link Query#from} option. If no fields are configured, the
   * {@link Query#fetch} call resolves with no data.
   * :::
   *
   * @example
   * ```js
   ...
   * User.fetch({
   *   from: {
   *     admin: User.query.where({ type: 'admin' }),
   *     external: User.query.sql.select('external_name').from('external')
   *   },
   *   fields: {
   *     admin: 'name',
   ...
   * SELECT
   *   "admin"."name",
   *   "external"."external_name" AS "name"
   * FROM
   *   (SELECT * FROM "user" WHERE "type" = 'admin') AS "admin",
   *   (SELECT "external_name" FROM "external") AS "external";
   * ```
   *
   * @param {object} from The value to use for the `FROM` clause. The object's
   * keys are used as the `FROM` aliases for their corresponding values.
   *
   * @returns {Query} The same {@link Query} instance to allow chaining.
   *
   * @todo debug/strict mode: throw if `from` is configured and `fields` isn't.
   */
  from(from) {
    this.options.from = Object.assign(this.options.from || {}, from);
  ...
  addFields(fields) {
    ...
    return this.appendOption('fields', fields);
  }

  // TODO: remove support for var-args
  // TODO: remove support for fields altogether in distinct?
  distinct(...fields) {
    if (fields[0] === false) {
   ...
   *
   * ::: tip INFO
   * When passed as `false`, no fields will be returned from the database call.
   *
   * When passed as an object, the keys are used as aliases while the values are
   * used in the query, which allows one to use raw SQL. However, when this
   * option is used in conjunction with the {@link Query#from} option, the top
   * level keys of the `fields` object correspond to the keys used in the
   * {@link Query#from} option.
   * :::
   *
   * ::: tip INFO
   ...
   * :::
   *
   * @param {string|array|boolean|object} fields The fields to return.
   *
   ...
   * ```
   *
   * @example When used in conjunction with a {@link Query#from} option:
   * ```js
   * Model.fetch({
   *  from: {
   *    user: 'user'
  ...
  formatAlias(alias, { quote } = {}) {
    if (this.config.schema) {
      alias = `${this.config.schema}.${alias}`;
    }
  ...
  getTable({ format = true, quote = true } = {}) {
    ...
    const alias = format
      ? this.formatAlias(this.config.alias, { quote })
      : this.config.alias;
  ...
  // TODO: strict mode: throw if field is unknown and is not an sql instance
  // TODO: breaking: default `quote` to `true` (breaking change - @knorm/paginate)
  // TODO: breaking: drop support for sql functions as raw strings and only support
  // sql bricks instances - allows us to validate field-names
  // TODO: breaking: remove options and instead call the functions where needed:
  // this.quote(this.formatColumn(this.getColumn(field)))
  ...
  getColumns(fields) {
    return Object.entries(fields).reduce((aliased, [alias, field]) => {
      ...
      const formattedColumn = this.config.from
        ? column
  ...
  _prepareFields(preparedFields, fields, fromAlias) {
    if (!isArray(fields)) {
      fields = [fields];
    }
    fields.forEach(field => {
      if (isArray(field)) {
        field.forEach(field => {
          preparedFields[`${fromAlias}.${field}`] = field;
        });
      } else if (isObject(field)) {
        Object.entries(field).forEach(([alias, field]) => {
          preparedFields[`${fromAlias}.${alias}`] = field;
        });
  ...
  prepareFields(sql) {
    ...
    const flattenedFields = fields.reduce((flattened, field) => {
      if (isArray(field)) {
        field.forEach(field => {
          flattened[field] = field;
        });
      } else if (isObject(field)) {
        Object.entries(field).forEach(([alias, field]) => {
          flattened[alias] = field;
        });
      } else if (isString(field)) {
        flattened[field] = field;
      }
    ...
    if (from) {
      Object.keys(from).forEach(alias => {
        const fromFields = flattenedFields[alias];
        if (!fromFields) {
          return;
        }
        this._prepareFields(preparedFields, fromFields, alias);
      });
    } else {
      this._prepareFields(preparedFields, fields, this.config.alias);
    }

    // TODO: for backwards compatibilty with @knorm/postgres and
    // @knorm/relations
    this.options.fields = flattenedFields;
    this.options.preparedFields = preparedFields;

    const columns = this.getColumns(preparedFields);
    const method = this.options.distinct ? 'distinct' : 'select';

    // TODO: fix this .select('') hack in sql-bricks either via .select(false)
    // or .select([])
    return sql[method](columns.length ? columns : '');
  }
  ...
  ensureFields() {
    if (!this.hasOption('fields')) {
      this.fields(this.config.fieldNames);
    }
  }
  ...
  // TODO: breaking: refactor prepareUpdateBatch => getUpdateBatch
  // depended on by knorm-postgres
  ...
  async getFrom() {
    ...
    if (!from) {
      ...
      return this.sql(`${table} AS ${this.getAlias()}`);
    }
    ...
    return Promise.all(
      Object.entries(from).map(async ([alias, value]) => {
        ...
        if (value instanceof Query) {
          value.config.from = true;
          // TODO: add `alias` option
          value.config.alias = alias;
          value = await value.prepareFetch();
        }

        const quotedAlias = this.quote(alias);

        if (value instanceof this.sql.Statement) {
          return value.as(quotedAlias);
        }

        return this.sql(`${this.quote(value)} AS ${quotedAlias}`);
      })
    );
  }
  ...
  parseRow(row) {
    ...
    Object.keys(this.options.preparedFields).forEach(alias => {
