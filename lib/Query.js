...
class Query extends WithKnex {
  ...
  fields(...fields) {
    return this.appendBuilderOption('columns', fields);
  }
  ...
  orderBy(...orderBy) {
    if (isObject(orderBy[0])) {
      ...
      this.appendBuilderOption('orderBy', orderBy);
    }
  ...
  transaction(transaction, options) {
    ...
    this.setBuilderOption('transacting', transaction);
    if (forUpdate) {
      this.setBuilderOption('forUpdate', forUpdate);
    }
    if (forShare) {
      this.setBuilderOption('forShare', forShare);
    }
  ...
  appendBuilderOption(option, value) {
  ...
  // TODO: test with raw param binding .raw('foo = :foo', { foo: 'bar' })
  ...
  prepareBuilderOption(option, values, options) {
    ...
    values = values.map(value =>
      this.formatBuilderOptionValue(option, value, options)
    );
    ...
    this.applyBuilderOptionValues(option, values);
  }
  ...
  async prepareBuilder(options) {
    ...
    if (forInsert || forUpdate || forDelete) {
      if (this.builderOptions.returning) {
        // TODO: `returning` is special-cased to allow calling it multiple times
        // with different values (not supported in knex)
        // the following should be: this.returning(this.primary);
        ...
        this.builderOptions.returning = [returning];
      } else {
        this.setBuilderOption('returning', '*');
      }
    } else {
    ...
    if (forFetch) {
      if (this.builderOptions.distinct) {
        ...
        if (this.builderOptions.columns) {
          this.columns(this.primary);
        } else {
          // TODO: this can be [['*']], but fieldNames are needed for joins
          this.setBuilderOption('columns', this.fieldNames);
        }
      }
    } else {
      delete this.builderOptions.columns;
...
builderOptions.forEach(option => {
  if (!Query.prototype[option]) {
    Query.prototype[option] = function(...args) {
      return this.appendBuilderOption(option, args);
    };
  }
});
