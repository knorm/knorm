...
class Query {
  ...
  getTable({ format = true, quote = true, alias = true } = {}) {
    const table = format
    ...
    const tableAlias = format
      ? this.formatTable(this.config.alias, { quote })
      : this.config.alias;

    return this.sql(`${table} as ${tableAlias}`);
  }
  ...
  // TODO: rename to `formatFieldAlias` (breaking change - @knorm/relations)
