...
class Query {
  ...
  // TODO: v2: refactor to `prepareInstance`
  ...
  prepareRowValue(field, value) {
    return value;
  }

  // depended on by @knorm/postgres
  prepareCastFields(fields) {
    return fields;
  }

  // TODO: v2: refactor to `prepareRow`
  ...
  async getRow(instance, options) {
    ...
    const castFields = this.prepareCastFields(fields, options);
    ...
    return Object.entries(data).reduce((row, [field, value]) => {
      ...
      row[column] = this.prepareRowValue({ field, column, value }, options);
      return row;
    }, {});
  }
