...
class Query {
  ...
  // TODO: default `quote`, `alias` options to `false` (breaking change)
  getTable({ quote = true, alias = true } = {}) {
    const table = quote ? this.quote(this.config.table) : this.config.table;

    if (!alias) {
      return table;
    }

    return this.config.table === this.config.alias
      ? table
      : this.sql(`${table} as ${this.quote(this.config.alias)}`);
  }

  // TODO: default `quote` to `false` (breaking change)
  formatColumn(column, { quote = true } = {}) {
    const alias = quote ? this.quote(this.config.alias) : this.config.alias;
  ...
  getColumns(fields) {
    return Object.entries(fields).reduce((aliased, [alias, field]) => {
      ...
      alias = this.formatAlias(alias, { quote: true });
