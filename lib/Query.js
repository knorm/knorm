...
class Query extends WithKnex {
  constructor(model) {
    ...
    this._fields = [{ field: this.model.primary }];
    this._returning = [{ field: this.model.primary }];
  ...
  _parseData(data, options) {
    ...
    const primary = instanceData[this.model.primary.name];
  ...
  async insert(data, options) {
    ...
    const prepareData = async data => {
      ...
      const primary = this.model.primary.name;
      if (instance[primary] === undefined) {
        fieldsNotToSave.push(primary);
      }
  ...
  async update(data, options) {
    ...
    const fieldsNotToSave = [this.model.primary.name];
  ...
  async save(data, options) {
    if (Array.isArray(data) || data[this.model.primary.name] === undefined) {
