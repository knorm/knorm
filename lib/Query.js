...
class Query extends WithKnex {
  constructor(model) {
    ...
    this.builder = this.knex(model.table);
  ...
  addUpdate(instance, fields) {
    const data = instance.getFieldData({ fields });

    Object.keys(data).forEach(fieldName => {
      const field = this.model.fields[fieldName];
      this.builder.update(field.column, data[field.name]);
    });
  }
  ...
  async update(data, options) {
    ...
    // TODO: strict mode: throw if there is no data to update is empty

    instance.cast({ fields: fieldsToUpdate, forSave: true });
    await instance.validate({ fields: fieldsToUpdate });

    this.addUpdate(instance, fieldsToUpdate);

    console.log(this.builder.toString());

    let rows;
    try {
      rows = await this.builder;
    } catch (error) {
