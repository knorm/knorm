...
class Query extends WithKnex {
  ...
  // TODO: pre-calculate these references and ensure they are correctly
  // updated when models are inherited
  _getReferences(fromModel, toModel) {
    const fromReferences = fromModel.config.references;

    if (!fromReferences) {
      return;
    }

    const fromFields = fromModel.config.fields;
    const references = Object.keys(fromReferences).reduce(
      (references, reference) => {
        const fromField = fromFields[reference];
  ...
  _join(type, queries, options) {
    ...
    queries.forEach(query => {
      ...
      const forwardReferences = this._getReferences(query.model, this.model);
      const reverseReferences = this._getReferences(this.model, query.model);
